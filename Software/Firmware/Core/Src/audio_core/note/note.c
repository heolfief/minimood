...
void update_envelope(Note *n, const Envelope *env, float sample_rate)
{
    ...
    if (n->master_onoff == ON)
    {
        ...
        {
            n->env_amp = (float) 1.0 + ((float) (n->lifetime - samples_attack)
                * ((env->sustain - (float) 1.0) / (float) samples_decay));     // Linear decrease from 1 to sustain
        }
        ...
        {
            n->env_amp =
                -(env->sustain / (float) samples_release) * (float) (n->lifetime - n->deathtime) + env->sustain;
